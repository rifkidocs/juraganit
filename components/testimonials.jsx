...
const reviews = [
  {
    name: "Elizabeth",
    username: "@DesignerUp",
    body: "I can't believe I just watched AI turn a boring text prompt into a complete UI design and flow right before my eyes in seconds with Uizard.",
    platform: "youtube",
  },
  {
    name: "Andre",
  ...
  },
  {
    name: "Nolan Perkins",
    username: "@_radnolan",
    body: "@uizard just dropped a Figma plugin and I think it's going to change my ui & ux design process! Uizard I think is going to help bridge the gap from idea to visualization which will help designers move faster, and product managers communicate better to their designers fostering better collaboration in the product team.",
    platform: "instagram",
  },
  {
    name: "Joelle",
    username: "@joelledesigner",
    body: "Uizard isn't your average AI tool that overpromises and under delivers - it has actually useful features that are designed to speed up your design process.",
    platform: "instagram",
  },
  {
    name: "Carl",
    username: "@carlvellotti",
    body: "I'm so freaking impressed with Uizard. Design tool + AI = Magic ✨",
    platform: "twitter",
  },
];

// Split reviews into three groups for the columns
const column1 = reviews.slice(0, 2);
const column2 = reviews.slice(2, 4);
const column3 = reviews.slice(4);
  ...
  return (
    <figure
      className={cn(
        ...
        "shadow-[0_-12px_25px_-12px_rgba(0,0,0,0.8)]", // Top shadow
        "shadow-[0_12px_25px_-12px_rgba(0,0,0,0.8)]" // Bottom shadow
      )}>
...
export function Testimonials() {
  return (
    <div>
      ...
      <div className='relative flex h-[600px] w-full justify-center gap-8 overflow-hidden bg-black px-4'>
        {/* Column 1 */}
        <Marquee vertical className='[--duration:40s] max-w-sm'>
          {[...column1, ...column1, ...column1].map((review, idx) => (
        ...
        <Marquee vertical reverse className='[--duration:35s] max-w-sm'>
          {[...column2, ...column2, ...column2].map((review, idx) => (
            <ReviewCard key={`${review.username}-${idx}`} {...review} />
          ))}
        </Marquee>
